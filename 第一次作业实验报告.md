...
| 内容 | 文件 |
| --- | --- |
| 第一部分 C 程序 | `first_assignment/part1_gcc/fib.c` |
| SysY 示例 | `first_assignment/part2_sysy_llvm_riscv/sysy_demo.sy` |
| LLVM IR | `first_assignment/part2_sysy_llvm_riscv/sysy_demo.ll` |
| 手写 RISC-V 汇编 | `first_assignment/part2_sysy_llvm_riscv/sysy_demo_asm.s` |
| 官方 VecAdd MLIR | `first_assignment/advanced_mlir/vecadd.mlir` |
| CPU 观察输入 | `first_assignment/advanced_mlir/vecadd_host_for_cpu.mlir` |

完整构建脚本和自动化测试脚本保存在对应实验目录中。

### 10.3 主要实验证据

| 模块 | 证据 |
| --- | --- |
| 第一部分编译阶段 | `first_assignment/part1_gcc/artifacts/fib.i`、`fib.s`、`fib.O2.s` |
| 第一部分目标文件和链接结果 | `fib_object_info.txt`、`fib_objdump.txt`、`fib_executable_info.txt` |
| 第一部分运行结果 | `first_assignment/part1_gcc/artifacts/tests/` |
| LLVM IR 运行结果 | `first_assignment/part2_sysy_llvm_riscv/artifacts/tests/` |
| 手写汇编运行结果 | `first_assignment/part2_sysy_llvm_riscv/artifacts/tests_asm/` |
| MLIR HIVM 中端证据 | `first_assignment/advanced_mlir/artifacts/` |

正文只保留关键片段和结果，完整内容以仓库中的源文件和运行日志为准。
